...
## Managing Permissioned Functions
...
Cross-chain transactions are executed from relayer accounts versus the user's wallet directly.  As a result, a `msg.sender` check will fail even if it _actually is_ the authorized wallet address sending the source chain transaction.
...
To authenticate wallets while enabling cross-chain execution of this arbitrary function, Songcamp included a hash of the user's wallet address in the transaction calldata and recovered the singer's address as follows:
