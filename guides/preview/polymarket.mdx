...
| `searchTags` | boolean  | No       | Search markets based on tags                                                                               | `true`                    |
...
```bash
curl -G "https://box-api-git-tw-staging-decent-webapp.vercel.app/api/workflows/polymarket/search?tags=Politics&tags=Sports&searchTags=true" \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY"
```
...
- The response follows the standardized response format with `events`, `tags`, and `pagination` fields
- Only markets with `acceptingOrders: true` can be used for orders
- The `clobTokenIds` field contains a JSON array of token IDs (use index 0 for "Yes", index 1 for "No")
